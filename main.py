...
def test(**kwargs):
    ...
    for i in range(len(pred)):
        for p in list(pred[i].numpy())[1:list(pred[i].numpy()).index(5)]:
            predict.append(p)
        for t in list(true[i].numpy())[1:list(true[i].numpy()).index(5)]:
            labels.append(t)
        # print(f"pred[i] : {list(pred[i].numpy())[1:list(pred[i].numpy()).index(5)]}")
        # print(f"true[i] : {list(true[i].numpy())[1:list(true[i].numpy()).index(5)]}")

    print(classification_report(y_true=labels, y_pred=predict, zero_division=0))
